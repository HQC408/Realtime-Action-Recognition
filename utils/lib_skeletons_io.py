...
class ReadValidImagesAndActionTypesByTxt(object):
    ...
    def read_image(self):
        ...
        if self.i > len(self.images_info):
            raise RuntimeError(f"There are only {len(self.images_info)} images, but you try to read the {self.i}th image")
        filepath = self.get_filename(self.i)
        print(f"Attempting to read image at path: {self.imgs_path + filepath}")  # Print out the path for debugging
